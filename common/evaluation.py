...
def sv_generate(net, dst, rows=10, cols=10, seed=0):
    """Visualization of singular value."""
    @chainer.training.make_extension()
    def make_image(trainer):
        ss = net.showOrthInfo()
        preview_dir = '{}/sv'.format(dst)
        preview_path = preview_dir + '/image{:0>8}.pdf'.format(trainer.updater.iteration)
        if not os.path.exists(preview_dir):
            os.makedirs(preview_dir)
        ...
        plt.legend()
        plt.savefig(preview_path, bbox_inches='tight',format="pdf", dpi = 300)
        plt.close()
    return make_image
...
def calc_inception(gen, isarray, dir, batchsize=100):
    @chainer.training.make_extension()
...
def calc_FID(gen, fidarray, dir, batchsize=100, stat_file="%s/cifar-10-fid.npz"%os.path.dirname(__file__)):
    ...
    def evaluation(trainer):
        ...
        plt.close()
        np.save(dir+"/fid_score.npy",{"FID_array":fidarray})
